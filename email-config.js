class EmailConfig {
    constructor() {
        ...
        // Initialize EmailJS immediately with defaults, then reload if envLoader completes
    ...
    loadCredentials() {
        this.publicKey = window.envLoader.get('sVP_L5YeJR4dPLBYp') || this.publicKey;
        this.serviceId = window.envLoader.get('service_68x1dfq') || this.serviceId;
        this.templateId = window.envLoader.get('template_qd5hgxo') || this.templateId;
        this.recipientEmail = window.envLoader.get('muthuselvamm022@gmail.com') || this.recipientEmail;
        ...
        console.log('EmailJS credentials loaded from environment:', {
            ...
            publicKey: this.publicKey ? this.publicKey.substring(0, 10) + '...' : '',
    ...
    initializeEmailJS() {
        if (window.emailjs && this.publicKey) {
            ...
            console.error('EmailJS not available or public key missing');
            this.isInitialized = false;
        }
    }
    ...
    getConfigStatus() {
        return {
            ...
            hasCredentials: Boolean(this.publicKey && this.publicKey !== 'YOUR_PUBLIC_KEY_HERE'),
            serviceId: this.serviceId,
            templateId: this.templateId,
